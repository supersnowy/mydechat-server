...
module.exports.createUser = async (req, res, next) => {
    try {
        ...
        const result = await user.create({ walletAddr, password, paidFee: true });
        res.locals = result;
        next();
    }
    catch (err) {
...
module.exports.updateUser = async (req, res, next) => {
    const { walletAddr, userName, phoneNo, status, dp } = req.body;
    try {
        let result = await user.findOneAndUpdate({ walletAddr }, { userName, phoneNo, status, dp }, { new: true });
...
module.exports.getUser = async (req, res, next) => {
    console.log('amk');
    const { walletAddr, password } = req.query;
    console.log(walletAddr);
    try {
        const result = await user.findOne({ walletAddr });
        if (result) {
...
module.exports.searchUser = async (req, res, next) => {
    try {
        ...
        // let result = await user.find({ walletAddr: { '$regex': '.*' + search + '.*' } }); 
        let result = await user.find({ $or:[ {walletAddr: { '$regex': '.*' + search + '.*' }}, {userName: { '$regex': '.*' + search + '.*' }}] }); 
        res.status(200).json({ sucess: true, payload: result });
    }
    catch (err) {
        next({ status: 400, message: err.message, stack: err.stack });
